...
class VLA0AsyncVLLMClient(nn.Module):
    ...
    async def _build_prompt_inputs(self, batch: dict[str, Tensor], index: int) -> tuple[dict, Tensor]:
        ...
        for key in present_img_keys:
            ...
            image = await loop.run_in_executor(None, functools.partial(self._process_image, img_tensor))
            images.append(image)
    ...
    async def _generate_actions_async(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        sampling_params = self._make_sampling_params(delta_output=True)
        sem = asyncio.Semaphore(self.config.vllm_max_concurrent_requests)

        async def process_single_sample_async(i: int) -> Tensor:
            inputs, state = await self._build_prompt_inputs(batch, i)

            async with sem:
                try:
                    generated_text = await self._generate_text_async(inputs, sampling_params)
                except Exception as e:
                    logging.error(f"Async vLLM error sample {i}: {e}")
                    raise RuntimeError("Async vLLM request failed") from e

            return self._decode_actions(generated_text, state)
    ...
    async def stream_actions_async(
        ...
        inputs, state = await self._build_prompt_inputs(batch, 0)
        sampling_params = self._make_sampling_params(delta_output=True)
        ...
        async for output in self.llm.generate(
            ...
            for completion in output.outputs:
                ...
                if not self._is_delta_output(sampling_params):
